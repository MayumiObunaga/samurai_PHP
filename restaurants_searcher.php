...
function write_data_to_csv($params){
    ...
    $handle = fopen("restaurants_list.csv", "wb"); //Windows環境(b)で書き込み可能なファイルを開く
    
    foreach ($restaurants as $values){
        fputcsv($handle, $values); //fputcsv(開かれたファイル, 書き込む要素);
    }

    fclose($handle);
    return print_r($restaurants); //変数に値を格納してわかりやすく表示
}
